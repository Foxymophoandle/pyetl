...
class WorkersManager(object):
    ...
    def __start_worker(self, worker):
        ...
        self._may_remove_data_from_jobs(worker)

    def _may_remove_data_from_jobs(self, worker):
        """
        When we start a worker, we check the items he is using. If noone
        else will be using them anymore, we reset the data in the completed jobs
        freeing memory
        """
        assert isinstance(worker, QueuedWorker)
        all_waited_workers = {w for w in worker.waiting_for for worker in self.workers_queue}
        for used_worker_name in worker.waiting_for:
            if used_worker_name not in all_waited_workers:
                self.logfile.info('manager - REMOVING [%s] DATA. NO ONE NEEDS IT ANYMORE' % used_worker_name)
                self.completed_jobs[used_worker_name] = None
